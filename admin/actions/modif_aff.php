<?php
	...
	if(!isset($_POST['id_article']) || !isset($_POST['contenu']))
	{
		...
		{	
			...
			{
				$contenu=recup_contenu_aff($_GET['id_article']);
				$view='views/form_modif_aff.php';
			}
			else
			{
		...
		{
			$erreur='L\'article est introuvable.';
			$view='views/rapport.php';
		}
	}
	else
	{
		$auteur=recup_auteur_aff($_POST['id_article']);
			
		if($auteur==$_SESSION['id'])
		{
			include 'models/modif_aff.php';
		}
		else
		{
			$erreur='Vous n\'avez pas les droits nécessaires pour modifier cet article.';
			$view='views/rapport.php';
		}
	}
